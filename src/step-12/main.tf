...
resource "openstack_compute_instance_v2" "workshop_worker" {
  count       = var.quantity
  name        = "${terraform.workspace}-workshop_worker-${count.index}"
  image_id    = "{{ image_id }}"
  ...
  block_device {
    uuid              = openstack_blockstorage_volume_v2.volume_config[count.index].id
    source_type       = "volume"
...
resource "null_resource" "provisions" {
  ...
  triggers = {
    cluster_instance_ids = openstack_compute_instance_v2.workshop_worker[count.index].id
  }

  # Bootstrap script can run on any instance of the cluster
  # So we just choose the first in this case
  connection {
    host         = openstack_compute_instance_v2.workshop_worker[count.index].network.0.fixed_ip_v4
    user         = "centos"
...
resource "null_resource" "provisions_configs" {
  count = var.quantity
  # Changes to  instance requires re-provisioning
  triggers = {
    cluster_instance_ids = openstack_compute_instance_v2.workshop_worker[count.index].id
  ...
  provisioner "file" {
    content      =  data.template_file.database_config.rendered
    destination = "/tmp/db.ini"
  }
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "chmod +x /tmp/provision.sh",
      "sudo cp /tmp/db.ini /config-data/db.ini",
    ]
  }
  depends_on = [
